...
CITY_CODE_CSV = os.path.join(
    BASE_DIR, "../CSV/city_code/99acres_city_code.csv")
LOCALITY_CODE_CSV = os.path.join(
    BASE_DIR, "../CSV/locality_code/99acres_locality_code.csv")
...
def solve():
    ...
    if not len(city_code_df):
        raise CityCodeException(
            "City not found in df. Please choose one of the city from the csv")
    ...
    city_url = f"https://99acres.com/search/property/buy?city={city_code}&availability={av}"
